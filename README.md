...
`vercel.json` selects the Next.js framework, `npm ci`, and `npm run vercel-build`. Configure the project for Node.js 22 and do not set a static output directory override; Vercel must deploy the Next.js application and its trusted Pages API functions together. Automatic Git deployments are enabled only for `main`; feature branches do not create persistent preview deployments.
...
3. Keep any explicitly created release-candidate environment isolated from production.
4. Build and verify the exact commit locally and in CI. Create a manual, temporary preview only when the approved test plan requires one.
5. Run the role-based checks in [QA_CHECKLIST.md](./QA_CHECKLIST.md).
6. Deploy only the exact reviewed commit reachable from `main` and verify the stable production alias.
7. Retain the prior production deployment only through the defined observation window, then delete superseded production and preview deployments so one canonical production deployment remains.
...
The landing page presents a fixed 3D lab lanyard with no editable fields or export controls. Inside the authenticated portal, the Digital ID page lets the signed-in user edit the card presentation, save it as an image, or print it. There are no shareable credential URLs, Web Share actions, LinkedIn/X actions, or alternate flat-card credential fallback. The artwork itself is marked as a user-customized display that is not valid for access. Saved images and printouts are not cryptographically signed credentials, physical access badges, proof of identity, or authorization tokens; every operational action remains subject to server authorization.
